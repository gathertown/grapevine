...
const IntercomOAuthCallbackPage: FC = () => {
  ...
  const [_redirectTo, setRedirectTo] = useState<string>('/integrations/intercom');
  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      ...
      if (error) {
        ...
        setTimeout(() => {
          navigate('/integrations/intercom?error=true');
        }, 3000);
        return;
      }
      ...
      try {
        ...
        // Use redirectTo from backend response, default to /integrations/intercom
        const destination = response.redirectTo || '/integrations/intercom';
        ...
        setStatus(CallbackStatus.Error);
        setErrorMessage('Failed to complete Intercom authorization. Please try again.');
        console.error('Failed to complete Intercom OAuth:', error);
      ...
      <Flex direction="column" mx="auto">
        <Flex direction="column" align="center" justify="center" gap={24}>
          ...
          <Text fontSize="lg" fontWeight="semibold">
            Completing Intercom authorization...
          </Text>
        </Flex>
      </Flex>
    );
    ...
    <Flex direction="column" maxWidth="800px" mx="auto" gap={24}>
      ...
      >
        <Text fontSize="md" fontWeight="semibold" color="dangerPrimary">
          Intercom Authorization Failed
        </Text>
        <Text fontSize="sm" color="secondary">
          {errorMessage}
        </Text>
      </Flex>
      <Button kind="primary" onClick={() => navigate('/integrations/intercom', { replace: true })}>
        Back to Intercom integration
      </Button>
    </Flex>
  );
};

export { IntercomOAuthCallbackPage };
