...
const FigmaIntegrationPage = () => {
  ...
  const integration = integrations.find((int) => int.id === 'figma');

  const isConnected = !!configData?.FIGMA_ACCESS_TOKEN;
  ...
  if (isLoading) {
    return (
      <Flex direction="column" gap={16} maxWidth="800px" mx="auto">
        <Text>Loading Figma configuration...</Text>
      </Flex>
    );
  }
  ...
  if (error || !configData) {
    return (
      <Flex direction="column" gap={16} maxWidth="800px" mx="auto">
        <Text color="dangerPrimary">Error loading Figma configuration</Text>
      </Flex>
    );
  }
  ...
  return (
    <Flex direction="column" gap={24} maxWidth="800px" mx="auto">
      ...
      <FigmaIntegration integration={integration} config={configData} />
      {oauthSuccess && (
        <Text fontSize="md" color="successPrimary">
          Figma connected successfully! Your design files will begin syncing shortly.
        </Text>
      )}
      {oauthError && (
        <Text fontSize="md" color="dangerPrimary">
          Figma connection failed. Please try again.
        </Text>
      )}
    </Flex>
  );
...
const FigmaIntegration = ({ config, integration }: FigmaIntegrationProps) => {
  return (
    <Flex direction="column" gap={24}>
      ...
      <Flex direction="column" gap={16}>
        <FigmaOauth config={config} />
      </Flex>
    </Flex>
  );
};

export { FigmaIntegrationPage };
