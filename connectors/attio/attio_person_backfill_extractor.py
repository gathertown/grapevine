"""Attio person backfill extractor."""
...
from connectors.attio.attio_artifacts import AttioObjectType, AttioPersonArtifact
from connectors.attio.attio_models import AttioPersonBackfillConfig
...
class AttioPersonBackfillExtractor(BaseExtractor[AttioPersonBackfillConfig]):
    """Extractor for processing Attio person backfill batch jobs.
    ...
    source_name = "attio_person_backfill"
    ...
    async def process_job(
        ...
        """Process a batch of Attio people for a tenant."""
        ...
        try:
            ...
            logger.error(f"Failed to process Attio people batch: {e}", exc_info=True)
            raise
        finally:
    ...
    async def _process_batch(
        ...
        """Process a batch of person records by ID."""
        tenant_id = config.tenant_id
        record_ids = config.record_ids

        logger.info(
            f"Processing batch of {len(record_ids)} Attio person records",
            tenant_id=tenant_id,
            backfill_id=config.backfill_id,
        )

        artifacts: list[AttioPersonArtifact] = []
        entity_ids: list[str] = []

        for record_id in record_ids:
            try:
                record = attio_client.get_record(
                    object_slug=AttioObjectType.PEOPLE.value,
                    record_id=record_id,
                )
                ...
                logger.error(
                    f"Failed to fetch person record {record_id}: {e}",
                    tenant_id=tenant_id,
                    record_id=record_id,
                )
                continue
        ...
        if artifacts:
            logger.info(f"Storing {len(artifacts)} person artifacts in batch")
            await self.store_artifacts_batch(db_pool, artifacts)

        # Trigger indexing for all processed records
        if entity_ids:
            logger.info(f"Triggering indexing for {len(entity_ids)} people")
        ...
        logger.info(
            f"Completed Attio people batch: {len(artifacts)} processed",
            tenant_id=tenant_id,
