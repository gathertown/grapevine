...
print_info "Deploying Jira Auth Proxy to $ENV environment"
...
# Set JIRA_APP_PAGE_TITLE, JIRA_GATEKEEPER_URL based on environment
case "$ENV" in
    production)
        JIRA_APP_PAGE_TITLE="Grapevine"
        JIRA_GATEKEEPER_URL="https://jira.ingest.getgrapevine.ai"
        ;;
    staging)
        JIRA_APP_PAGE_TITLE="Grapevine [staging]"
        JIRA_GATEKEEPER_URL="https://jira.ingest.stg.getgrapevine.ai"
        ;;
    local)
        JIRA_APP_PAGE_TITLE="Grapevine [local]"
        ...
        JIRA_GATEKEEPER_URL="$GATEKEEPER_URL"
        ;;
esac

print_info "Using JIRA_APP_PAGE_TITLE: $JIRA_APP_PAGE_TITLE"
print_info "Using JIRA_GATEKEEPER_URL: $JIRA_GATEKEEPER_URL"
...
export JIRA_APP_PAGE_TITLE
export JIRA_GATEKEEPER_URL
...
if forge deploy -e production; then
    print_success "Successfully deployed Jira Auth Proxy to $ENV environment!"
else
