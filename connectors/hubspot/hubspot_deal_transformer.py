"""
HubSpot transformer for converting HubSpot deal artifacts to documents.
"""
...
from connectors.hubspot.hubspot_artifacts import HubspotDealArtifact
from connectors.hubspot.hubspot_deal_v2_document import HubspotDealDocument
...
class HubSpotDealTransformer(BaseTransformer[HubspotDealDocument]):
    """Transformer for HubSpot deal artifacts."""

    def __init__(self):
        super().__init__(DocumentSource.HUBSPOT_DEAL)

    async def transform_artifacts(
        self, entity_ids: list[str], readonly_db_pool: asyncpg.Pool
    ) -> list[HubspotDealDocument]:
        """Transform HubSpot deal artifacts into documents.

        Args:
            entity_ids: List of deal IDs to transform
            readonly_db_pool: Database connection pool

        Returns:
            List of HubspotDealDocument instances
        """
        repo = ArtifactRepository(readonly_db_pool)

        # Get HubSpot deal artifacts for the given entity IDs
        artifacts = await repo.get_artifacts_by_entity_ids(HubspotDealArtifact, entity_ids)

        logger.info(
            f"Loaded {len(artifacts)} HubSpot deal artifacts for {len(entity_ids)} entity IDs"
        )

        deal_custom_properties = await self._get_deal_custom_properties(readonly_db_pool)

        logger.info(
            f"Loaded {len(deal_custom_properties)} deal custom properties for {len(entity_ids)} entity IDs"
        )
        ...
        for artifact in artifacts:
            with record_exception_and_ignore(
                logger, f"Failed to transform HubSpot deal artifact {artifact.id}", counter
            ):
                document = self._create_document(artifact, deal_custom_properties)
    ...
    async def _get_deal_custom_properties(
    ...
    def _create_document(
        self, artifact: HubspotDealArtifact, deal_custom_properties: list[HubSpotProperty]
    ) -> HubspotDealDocument:
        """Create a HubspotDealDocument from an artifact.

        Args:
            artifact: The HubspotDealArtifact to transform
        ...
        # Generate document ID using the deal ID (entity_id)
        document_id = get_hubspot_doc_id("deal", artifact.entity_id)
        ...
        # Add deal custom property data
        raw_data["custom_properties"] = {
            property.name: property.label for property in deal_custom_properties
        }
        ...
        raw_data["activities"] = content.get("activities")
